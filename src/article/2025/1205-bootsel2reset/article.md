...
長押し書き込みボタンは便利なんですが、毎回複数の部品を取り付ける作業が
地味にめんどいので既存の BOOTSEL スイッチを使って同様の機能を実現する
MOD チップ的なものを作ってみることにしました。
...
- BOOTSEL の t<sub>CLICK</sub> 未満のパルスは無視
- BOOTSEL の t<sub>CLICK</sub>～t<sub>HOLD</sub> のパルスでは立ち上がりでリセットパルスを生成
...
押す時間は TIMESEL ピンで選択できます。

|TIMESEL|クリック|長押し (t<sub>HOLD</sub>)|
|:--|:--|:--|
|オープン|1000 ms|5000 ms (4900 ms)|
|GND に接続|100 ms|500 ms (400 ms)|
