...
ハードウェア的には他の汎用レジスタと同様に機能するが、基本的には常に 0x00 を格納しておいて、定数ゼロとして使用することになっている。乗算命令 (`mul` など) の結果は `r1:r0` に格納されるので、乗算結果を使用した後はゼロクリアする必要がある。
...
- 構造体でない戻り値は引数と同様、サイズを偶数に切り上げる。
    ...
    - 例) `uint32_t` の引数は `r25:r22` に配置する (`r25` が MSB 側)。

- 構造体は 2 のべき乗に切り上げる。

    - 例) 6 バイトの構造体は `r23:r19` に配置する (`r25:r24` はパディング)。
